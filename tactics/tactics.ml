...
type eliminator =
| ElimConstant of EConstr.constr * equality_position_on_elim
...
let general_elim_clause0 with_evars flags (submetas, c, ty) elim =
  ...
  | ElimConstant (elimc, i) ->
    ...
    let i = match i with
    | AtPosition i -> i
    | UnknownPosition -> index_of_ind_arg sigma elimt
    in
...
let general_elim_clause with_evars flags where arg elim indarg =
  match where with
  | None -> general_elim_clause0 with_evars flags arg (ElimConstant (elim, indarg))
  | Some id -> general_elim_clause_in0 with_evars flags id arg elim indarg
...
let intro_decomp_eq ?loc l thin tac id =
  ...
  | Some (eq,u,eq_args) ->
    !intro_decomp_eq_function
      ...
      (EConstr.mkIndU (Globnames.destIndRef eq.eq, u),eq.congr,t,eq_args) (c, t)
  | None ->
    let info = Exninfo.reify () in
