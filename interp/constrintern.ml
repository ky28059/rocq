...
let intern_restart_binders self genv env = intern self genv (restart_lambda_binders env)
...
let intern_disjunctive_multiple_pattern genv env ntnvars loc n mpl =
  assert (not (List.is_empty mpl));
  let mpl' = List.map (intern_multiple_pattern genv env ntnvars n) mpl in
      ...
      let (match_to_do,nal) =
        let rec canonize_args case_rel_ctxt arg_pats forbidden_names match_acc var_acc =
          ...
          | (LocalAssum (cano_name,ty) | LocalDef (cano_name,_,ty)) :: t, c::tt ->
            ...
            | _ ->
              let fresh =
                Namegen.next_name_away_with_default_using_types genv (Evd.from_env genv)
                  "iV" cano_name.binder_name forbidden_names (EConstr.of_constr ty) in
...
let intern_proj self genv env lvar ?loc expl (qid,us) args1 c args2 =
  ...
  | Some (p, us, args0, nexpectedparams) ->
    ...
    let head, impls, subscopes = find_appl_head_data genv env lvar f in
    let imps1, imps2 =
      if expl then
        [], []
      else
...
let cases self genv env lvar ?loc (sty, rtnpo, tms, eqns) =
  ...
  let env' = Id.Set.fold
      (fun var bli -> push_name_env ~dump:true (snd lvar) [] bli (CAst.make @@ Name var))
